...
export default class ColorAttribution extends BaseCommand {
  public static commandName = 'color:attribution'
  public static description = 'Run color detection on all painting products'
  ...
  public async run() {
    logTaskBoundary(true, 'Color Attribution')

    // ====================================================================
    // CONFIGURATION
    // ====================================================================
    // Set to true to see what would be processed without actually running color detection
    const DRY_RUN = false
    ...
    // Skip products that already have colors set
    const SKIP_EXISTING_COLORS = true
    // ====================================================================

    console.info(`🎨 Color Attribution - Batch Processing`)
    console.info(`${'='.repeat(60)}`)
    console.info(`Mode: ${DRY_RUN ? '🔍 DRY RUN (no changes)' : '✅ LIVE (will update products)'}`)
    console.info(`Skip existing colors: ${SKIP_EXISTING_COLORS ? 'Yes' : 'No'}`)
    if (MAX_PRODUCTS) {
    ...
    try {
      ...
      if (DRY_RUN) {
        ...
        logTaskBoundary(false, 'Color Attribution')
        return
      }

      // Step 4: Process each product
      console.info(`\n${'═'.repeat(60)}`)
      console.info(`🚀 Starting color detection...`)
      console.info(`${'═'.repeat(60)}\n`)
      ...
      for (let i = 0; i < productsToProcess.length; i++) {
        ...
        try {
          ...
          // Check if should skip (colors already set)
          if (SKIP_EXISTING_COLORS) {
            // This check will be done inside detectAndSetColors, but we can log it here
            const hasColors = fullProduct.metafields?.edges.find(
              (edge) => edge.node.namespace === 'shopify' && edge.node.key === 'color-pattern'
            )
            if (hasColors) {
              console.info(`⏭️  Skipped: Colors already set`)
              results.skipped++
              continue
            }
          }
          ...
          // Run color detection
          await chatGPT.colorPattern.detectAndSetColors(fullProduct)
      ...
      console.info(`⏭️  Skipped (has colors):  ${results.skipped}`)
      ...
      if (results.processed > 0) {
        console.info(`\n🎉 Color attribution completed successfully!`)
      }
    } catch (error: any) {
