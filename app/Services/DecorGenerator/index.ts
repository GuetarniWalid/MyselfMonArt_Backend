...
OUTPUT: lowercase, 6-16 words, comma-separated descriptors (palette + materials + a short mood, optionally one signature touch OR one explicitly-requested real everyday object named as a plain real-world noun, e.g. "vintage vinyl record" — never both). No sentences, no preamble, no quotes, no labels, no markdown, no explanation. English only.
...
- LISTEN TO AN EXPLICIT OBJECT REQUEST (refine, do not denature): when the user explicitly names a CONCRETE, REAL, EVERYDAY, tasteful object that belongs in a premium European interior (e.g. a vintage vinyl record, a stack of art books, a hand-thrown ceramic vase, a glass carafe, a brass candlestick, a wool throw, a film camera, a chess set, a potted olive tree), KEEP IT as ONE named object in the brief instead of dissolving it into pure palette/material abstraction. Append a single short clause naming it and placing it low/to the side, e.g. ", one vintage vinyl record on a low shelf" or ", a stack of art books on the floor". Refine its wording, material/finish, era and placement; never delete it — the user asked for it, honour it. This named object REPLACES the optional signature touch: keep exactly ONE extra element total, never both a named object and a separate signature touch.
- OBJECT ELIGIBILITY (keep the object ONLY if ALL hold): (a) it is a real, recognizable, physically plausible everyday/design object — not invented, not a hybrid, not surreal, not oversized; (b) it sits as a small ROOM accent, low and to the side, never inside/on/behind-touching the empty grey frame; (c) it is not kitsch, not a cultural/regional/religious/touristy/era cliche, not a flag/mask/idol/branded antique, not a person/animal/text/screen/second frame/mirror. If the requested object FAILS any of these, fall back to the existing rules: re-route a frame-violating request to a room accent, and translate a cliche/kitsch/cultural object into European materials, tones and light only — do NOT name it. A vintage vinyl record, art books or a ceramic vase PASS; "a Buddha statue", "an Eiffel-Tower miniature", "a dreamcatcher", "a neon sign", "a TV" FAIL. NEVER introduce a concrete object the user did not request — if the wish is only a mood/palette, name NO object and stay material/tone/light only.
- DE-CLICHE aggressively: "boho/boheme" -> warm earth tones, raw linen, matte ceramic, soft texture (NOT macrame, mandalas, dreamcatchers, ethnic motifs); "mediterranean" -> sun-warmed limewash, terracotta, pale stone, olive-grey linen (NOT blue-white tiles, fishing nets); "vintage/retro" as a STYLE -> patinated brass, aged oak, muted ochre, soft grain (NOT branded antiques, sepia, knick-knacks); BUT if the user names a specific real vintage OBJECT (e.g. a vinyl record, a film camera), keep that one object named per the object rule above while still styling everything else this way; "scandinave" -> pale oak, soft greige, wool, restraint; "vibrant/colorful/warm" -> one warm muted accent + warm directional light (NOT rainbow or high saturation).
...
COMPOSITION & STYLING (controlled asymmetry, lived-in, never cluttered): the ${productNoun} stays centered and the undisputed focal point, but the surrounding room is arranged in a relaxed off-center triangle so the space breathes and feels real rather than mirror-symmetric and lifeless. A few deliberate, high-taste props as honest signs of life, placed LOW and to the SIDES — never on the feature wall, never near or overlapping the piece: one sculptural ceramic, a single stem in a hand-thrown vase, a stack of design books, a textured accent — plus, if the styling direction explicitly named a specific real object, that exact object shown clearly and correctly. Every object in the room is an actual, identifiable, real-world everyday item, correctly formed, complete and plausibly proportioned in scale, structure and material; never invent, hallucinate, fuse, duplicate, melt or distort objects, and never add ambiguous blob-like or half-formed props — if an object cannot be rendered cleanly and recognizably, omit it and fall back to material, texture and shadow rather than guess. Restraint and negative space are luxuries — curated, breathing, alive, with real human presence implied but no people.

${themeBlock}

Negative: empty soulless room, flat sourceless lighting, beige-on-beige plastic, dead symmetry, generic stock blandness; no people; exactly one ${productNoun}, no second frame, no gallery wall, no mirror, no TV or screen; nothing inside the ${productNoun}; no shadow, gradient or glare on the grey surface; the signature gesture must not touch, overlap or shadow the piece; no invented, surreal, impossible or fantastical objects; no melted, fused, hybrid, half-formed, malformed, deformed or warped objects; no duplicated, extra or floating objects; no unidentifiable items, nonsensical props, abstract blobs masquerading as objects, garbled detail, broken geometry or AI artifacts; every object must be a real, recognizable everyday item, correctly shaped and proportioned; no text, no signature, no watermark, no logo, no border.`
}
