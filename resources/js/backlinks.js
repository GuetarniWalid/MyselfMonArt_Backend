...
function backgroundClass(status) {
    switch (status) {
        case 'ok':
            return { class: 'gridjs-td !bg-emerald-200 cursor-default max-w-[150px] truncate relative group' }
        case 'ko':
            return { class: 'gridjs-td !bg-red-200 cursor-default max-w-[150px] truncate relative group' }
        default:
            return { class: 'gridjs-td !bg-stone-100 cursor-default max-w-[150px] truncate relative group' }
    }
}
...
function formatOutput(cell, _, column) {
    ...
    if (cell === true) {
        ...
        content = cell === '?' ? `<span class='w-full block text-center cursor-default'>?</sapn>` : `<span>${cell}</span>`
        content = html(`
        ${content}
...
function isValidUrl(url) {
    const pattern = new RegExp('^(https?:\\/\\/)' + // protocole (obligatoire)
...
async function deleteLinkElement(e, cells) {
    try {
        ...
        if (isdeleteButton) {
            ...
            loadingButton.classList.add('absolute', 'top-1/2', '-translate-y-1/2', 'right-2', 'bg-gray-200/70', 'text-gray-700', 'p-2', 'cursor-pointer', 'hover:bg-gray-300', 'rounded');
...
/***** Events *****/
document.getElementById('add-row').addEventListener('click', () => updateGridData({ type: 'input', value: '' }))
